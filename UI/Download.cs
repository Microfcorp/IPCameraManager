...
namespace IPCamera.UI
{
    public partial class Download : Form
    {
        ...
        {
            Log = "Идет загрузка...";            
            LoadingForm lf = new LoadingForm(false, "Загрузка файлов с камеры...");
            if (showdownload)
            {
                lf.Show();
            ...
            if (showdownload) { lf.Close(); Log = "Загрузка завершена"; }
            return list.ToArray();
        }
        ...
        private void просмотрToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else if(selected.typeNode == FileTreeNode.TypeNode.File && selected.URL.EndsWith(".264"))
            {
                LoadingForm lf = new LoadingForm(false, "Получение файла с камеры...");
                lf.Show();
                lf.PointToCenter(Application.OpenForms[0]);
                lf.Update();
